...
use App\Adapter\WooCommerceOrderItemAdapter;
use PHPUnit\Framework\TestCase;

class WooCommerceOrderItemAdapterTest extends TestCase
{
  private WooCommerceOrderItemAdapter $WCOrderItemAdapter;
  ...
  {
    $item = json_decode($this->JSON_ITEMS, true);

    $this->WCOrderItemAdapter = new WooCommerceOrderItemAdapter($item);
  }
  ...
  public function testObject1()
  {
    ...
    JSON;
    $item = json_decode($JSON_ITEMS, true);

    $WCOrderItemAdapter = new WooCommerceOrderItemAdapter($item);

    self::assertSame(380, $WCOrderItemAdapter->getMenuNum());
    self::assertSame('25.00', $WCOrderItemAdapter->getUnitPrice());
    self::assertSame(1, $WCOrderItemAdapter->getQuantity());
    self::assertSame("38.加大Extra Large (+&#36;1.50) -> L\n" .
      "加菜，蛋，肉，面 Extras (+&#36;5.50) -> 132.加肉.../份Extra Meat\n" .
      "370.腊汁肉机切面 (+&#36;1.50) -> 腊汁肉机切面\n"
      , $WCOrderItemAdapter->getNote());
  }
  ...
  public function testObject2()
  {
    ...
    self::assertSame(301, $WCOrderItemAdapter->getMenuNum());
    self::assertSame("33.21", $WCOrderItemAdapter->getUnitPrice());
    self::assertSame(13, $WCOrderItemAdapter->getQuantity());
    self::assertSame(""
      , $WCOrderItemAdapter->getNote());
  }

  public function testGetMenuNumFromItemName()
  {
    $names = [
    ...
    foreach ($names as $name => $expected) {
      ...
      $actual = $this->WCOrderItemAdapter->getMenuNumFromItemName($name);
      $this->assertEquals($expected, $actual);

    }
  }

  public function testResolveNoteEmpty()
  {
    $actual = $this->WCOrderItemAdapter->resolveNote(null);
    $this->assertSame("", $actual);
  }

  public function testResolveNoteLong()
  {
    $meta_data = [
      [
        'id' => 38,
    ...
    $actual = $this->WCOrderItemAdapter->resolveNote($meta_data);
    $this->assertSame("38.加大Extra Large (+&#36;1.50) -> L\n" .
      "加菜，蛋，肉，面 Extras (+&#36;5.50) -> 132.加肉.../份Extra Meat\n" .
      "370.腊汁肉机切面 (+&#36;1.50) -> 腊汁肉机切面\n"
      , $actual);
  }

  public function testResolveNoteOneLine()
  {
    $meta_data = [
      [
        'id' => 370,
    ...
    $actual = $this->WCOrderItemAdapter->resolveNote($meta_data);
    $this->assertSame("370.腊汁肉机切面 (+&#36;1.50) -> 腊汁肉机切面\n"
      , $actual);
  }

  public function testCheckUpgradeOption1()
  {
    $JSON_ITEM = <<<'JSON'
      {
        "product_id": 100001,
    ...
    $actual = $this->WCOrderItemAdapter->checkUpgradeOption($item['meta_data'], 37);

    $this->assertEquals(380, $actual);

  }

  public function testCheckUpgradeOption2()
  {
    $JSON_ITEM = <<<'JSON'
      {
        "product_id": 37,
  ...
  public function testCheckUpgradeOption3()
  {
    $JSON_ITEM = <<<'JSON'
      {
        "product_id": 37,
  ...
  public function testCheckUpgradeOption4()
  {
    $JSON_ITEM = <<<'JSON'
      {
        "product_id": 37,
      ...
      JSON;
    $item = json_decode($JSON_ITEM, true);
    $actual = $this->WCOrderItemAdapter->checkUpgradeOption($item['meta_data'], $item['product_id']);
  ...
  {
    $actual = $this->WCOrderItemAdapter->getMenuNumString();
    $this->assertSame("37.腊汁肉干拌面", $actual);
  }
}
